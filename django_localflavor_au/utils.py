...
def abn_validator(value):
    ...
    if re.search(r'[^0-9]', value):
        raise ValidationError(u'Only digits are allowed in an ABN')

    # ABNs are 11 digits long
    if len(value) != 11:
        raise ValidationError(
            _(u'An ABN must have 11 digits, "{0}" has {1}').format(
                value, len(value)))
    ...
    if remainder != 0:
        raise ValidationError(
            u'This is not a valid ABN. Check you have entered the number '
            u'correctly, and try again.')
...
def acn_validator(value):
    ...
    if re.search(r'[^0-9]', value):
        raise ValidationError(u'Only digits are allowed in an ACN')

    # ACNs are 9 digits long
    if len(value) != 9:
        raise ValidationError(
            _(u'An ACN must have 9 digits, "{0}" has {1}').format(
                value, len(value)))
    ...
    if complement != check:
        raise ValidationError(
            u'This is not a valid ACN. Check you have entered the number '
            u'correctly, and try again.')
